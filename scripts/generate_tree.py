...
# 投稿の切り口バリエーション。毎回ここからランダムに選び、文面の固定化を防ぐ。
ANGLES = [
    ("逆張り", "「実は〜しない方がいい」「みんな誤解してる」という常識への反論から入る"),
    ("失敗談", "筆者自身がやらかした・損した実体験の告白から入る"),
    ("数字ショック", "意外な数字・金額・データを冒頭にドンと置いて驚かせる"),
    ("読者への質問", "「〜で困ってませんか？」「〜したこと、ある？」という問いかけから入る"),
    ("ビフォーアフター", "やる前とやった後の変化を具体的に対比して見せる"),
    ("常識のアップデート", "「昔はそれが正解だったけど、今は違う」という時代の変化を軸にする"),
    ("チェックリスト", "「これに当てはまる人は要注意」と条件を提示して自分ごと化させる"),
    ("たとえ話", "身近なものへの比喩・たとえ話で核心をわかりやすく説明する"),
    ("時短・コスパ", "「結局いくら得するか」「何分浮くか」という損得勘定に振り切る"),
    ("ひとこと本音", "「正直に言うと…」と本音・ぶっちゃけトーンで語り始める"),
]
...
- 指定された【切り口】を必ず1投稿目のフックに使うこと。
- 毎回同じ書き出し・同じ言い回しを避け、切り口に合わせて文体・リズムも変えること。
...
def call_claude(article: dict, angle: tuple[str, str] | None = None) -> dict:
    ...
    print(f"[angle] {angle_name}", file=sys.stderr)

    user = f"""今回の切り口: 【{angle_name}】 — {angle_desc}
この切り口で1投稿目のフックを作ってください。
